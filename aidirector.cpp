...
Director::Director( Map *pMap ) : m_RandomNumberGenerator ( 4 )
{
    if ( !pMap )// Handle this
    {
        m_mMap = new map();
                ...
                if ( pPerson->GetReproductionValue() >= PERSON_REPRODUCTION_THRESHOLD )
                {
                    ...
                    for ( int iXOffset = -1; iXOffset <= 1; iXOffset++ )
                    {
                        for ( int iYOffset = -1; iYOffset <= 1 ; iYOffset++ )
                        {
                            ...
                            pEmptyCell = m_mMap->UTIL_At2D( m_pMap, iHorizontalPosition + iXOffset, iVerticalPosition + iYOffset );
...
EmptyCellFound:
                ...
                if ( IsNumberInRange( iDesiredYPosition, 0, ( int )iMapHeight ) && IsNumberInRange( iDesiredXPosition, 0, ( int )iMapWidth) )
                {
                    pTargetCell = m_mMap->UTIL_At2D( m_pMap, iDesiredXPosition, iDesiredYPosition );
                }
